# English from Artu. Obsidian Vault, powered by Quartz.

English grammar and usage notes published with [Quartz 4](https://quartz.jzhao.xyz/). Keep adding or editing Markdown files and Quartz will handle the navigation automatically.
...
1. Push the repo to GitHub (e.g. `artu/English`).
...
If you host in another repo, update `homepage`/`repository` in `package.json` and `baseUrl` in `quartz.config.ts` (currently `artu.github.io/English`).

## Adding new notes

1. Create a file in `content/` (e.g. `content/conditionals.md`).
